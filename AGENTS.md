...
- **Observability:** [`autofill/telemetry.py`](autofill/telemetry.py) handles both PostHog (anonymous usage events, **opt-in** via `AUTOFILL_TELEMETRY=1`) and Sentry (crash reports, **opt-in** via `AUTOFILL_SENTRY=1`). Both default to off: this tool reads resumes and identity documents, so no data leaves the machine without an explicit yes. PostHog is offered as a setup question (`_onboard_telemetry`, default no, answer written to `.env`); Sentry is env-var only and deliberately never prompted for, because its stack frames can incidentally capture profile data. `init_sentry()` is called from `cli()` right after `load_dotenv()`.
