...
```bash
autofill                    # first time: runs onboarding (profile, key, files, DB)
autofill <form-url>         # fill a form
autofill --provider anthropic <url>   # after uv sync --extra anthropic + ANTHROPIC_API_KEY
```
